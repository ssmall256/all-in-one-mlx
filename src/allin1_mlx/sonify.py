...
def _load_audio(path) -> np.ndarray:
  # mlx-audio-io returns [frames, channels] by default
  # Built-in resampling is faster than librosa (4.6x speedup)
  data, sr = mlx_load_audio(str(path), sr=_AUDIO_SR, dtype="float32")
  # Convert MLX array to NumPy and transpose to [channels, frames]
  wav = np.array(data, copy=False).T
  return wav
...
def sonify(
  ...
  use_pool = multiprocess and len(results) > 1
...
def _sonify(
  ...
  if out_dir is not None:
    ...
    save_audio(
      wav=mixed,
      path=out_dir / f'{result.path.stem}.sonif.wav',
      samplerate=sr,
    )
